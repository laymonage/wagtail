...
class GroupApprovalTask(Task):
    ...
    def _user_in_groups(self, user):
        ...
        attr = "_group_approval_task_checks"
        if not hasattr(user, attr):
            setattr(user, attr, {})

        if self.pk not in user._group_approval_task_checks:
            user._group_approval_task_checks[self.pk] = self.groups.filter(
                pk__in=user.groups.values_list("id", flat=True)
            ).exists()

        return user._group_approval_task_checks[self.pk]
