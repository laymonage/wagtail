...
from wagtail.test.testapp.models import RevisionModel, SimplePage
...
class TestRevisionModel(TestCase):
    ...
    def test_content_type_without_inheritance(self):
        self.instance.text = "updated"
        revision = self.instance.save_revision()
        content_type = ContentType.objects.get_for_model(RevisionModel)
        ...
        self.assertEqual(revision, revision_from_db)
        self.assertEqual(self.instance.get_base_content_type(), content_type)
        self.assertEqual(self.instance.get_content_type(), content_type)
