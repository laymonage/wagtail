...
class CollectionPermissionPolicy(
    ...
    def annotate_with_permissions(self, queryset, user, actions):
        ...
        # Skip annotation if we already know whether the user has permission
        if default_access is None:
            queryset = queryset.annotate(
                **{
                    f"_w_can_{action}": Exists(
        ...
        for instance in queryset:
            instance.user_permissions = {
                ...
                else getattr(instance, f"_w_can_{action}")
                for action in actions
            }
        return queryset
...
class CollectionOwnershipPermissionPolicy(
    ...
    def annotate_with_permissions(self, queryset, user, actions):
        ...
        if known_access is not None:
            for instance in queryset:
                instance.user_permissions = {action: known_access for action in actions}
            return queryset
        ...
        def permission_exists_query(action):
            return Exists(
                GroupCollectionPermission.objects.filter(
                    group__user=user,
                    permission__content_type=self._content_type,
        ...
        for action in set(actions) - action_aliases.keys():
            if action == "change":
                queryset = queryset.annotate(
                    w_can_change=ExpressionWrapper(
                        (
                            # user has 'add' permission and is the owner
                            (
                                permission_exists_query("add")
                                & Q(**{self.owner_field_name: user})
                            )
                            # OR user has 'change' permission
                            | permission_exists_query("change")
                        ),
                ...
                queryset = queryset.annotate(
                    **{f"_w_can_{action}": permission_exists_query(action)}
                )
