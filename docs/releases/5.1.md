...
### `UserPagePermissionsProxy` methods are now cached properties

The methods in the undocumented `UserPagePermissionsProxy`, other than `for_page()`, are now cached properties. If you have been using any of the `revisions_for_moderation`, `explorable_pages`, `editable_pages`, `can_edit_pages`, `publishable_pages`, `can_publish_pages`, and `can_remove_locks` methods, you should now replace the method call with a property access, e.g. `user_perms.editable_pages()` becomes `user_perms.editable_pages`.
